...
/*
 * Timer trigger possibilities (STM32F1)
 *
 *       TS 000   001    010    011
...
/*
 * STM32F2 and STM32F4 TIM2 implements a 32-bit counter. This allows us
 * to very easily implement a 64-bit clock tick value by software
 * incrementing the top 32 bits on the 32-bit rollover every ~72 seconds.
 *
...
timer_init(void)
{
    ...
    TIM_CR1(TIM3) &= ~(TIM_CR1_CKD_CK_INT_MASK | TIM_CR1_CMS_MASK |
                       TIM_CR1_DIR_DOWN);
    TIM_CR1(TIM1) &= ~(TIM_CR1_CKD_CK_INT_MASK | TIM_CR1_CMS_MASK |
                       TIM_CR1_DIR_DOWN);
...
timer_init(void)
{
    ...
    TIM_CR1(TIM2) &= ~(TIM_CR1_CKD_CK_INT_MASK | TIM_CR1_CMS_MASK |
                       TIM_CR1_DIR_DOWN);
...
{
    uint64_t ticks_per_usec = rcc_apb2_frequency / 1000000;  // nominal 60 MHz
...
{
    return (rcc_apb2_frequency / 1000000 * nsec / 1000);
}
...
{
    return (value / (rcc_apb2_frequency / 1000000));
}
...
{
    uint64_t ticks_per_msec = rcc_apb2_frequency / 1000;
...
{
    uint64_t ticks_per_usec = rcc_apb2_frequency / 1000000;
