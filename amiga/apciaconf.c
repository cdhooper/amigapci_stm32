#include <exec/types.h>
#include <libraries/configvars.h>
#include <libraries/configregs.h>
#include <proto/exec.h>
#include <proto/dos.h>
#include <inline/dos.h>
#include <inline/exec.h>
#include <proto/expansion.h>
#include <inline/expansion.h>

struct ExpansionBase *ExpansionBase = NULL;

#define DEVICE_MFG  0x0e3b  // E3B
#define DEVICE_PROD 0xc8    // Prometheus Firestorm

static int firestorm_mode = 0;

static void
usage(void)
{
    Printf("apciaconf options\n"
           "   -f fake Firestorm bridge for A3000/A4000\n");
}

int
main(int argc, char *argv[])
{
    struct ConfigDev *cd;
    int arg;

    for (arg = 1; arg < argc; arg++) {
        char *ptr = argv[arg];
        if (*ptr == '-') {
            for (++ptr; *ptr != '\0'; ptr++) {
                switch (*ptr) {
                    case 'f':  // Firestorm mode
                        firestorm_mode = 1;
                        break;
                    default:
                        Printf("Unknown argument %s\n", (ULONG)ptr);
                        usage();
                        return (1);
                }
            }
        } else {
            Printf("Error: unknown argument %s\n", (ULONG)ptr);
            usage();
            return (1);
        }
    }

    /* Open expansion library */
    ExpansionBase = (void *)OpenLibrary("expansion.library", 37);
    if (!ExpansionBase) {
        Printf("Failed to open expansion.library\n");
        return (1);
    }

    cd = FindConfigDev(NULL, DEVICE_MFG, DEVICE_PROD);
    if (cd != NULL) {
        Printf("Mfg $%04lx  Prod $%02lx  already exists\n",
               DEVICE_MFG, DEVICE_PROD);
        goto done;
    }

    /* Allocate memory for a new ConfigDev structure */
    cd = AllocMem(sizeof(struct ConfigDev), MEMF_PUBLIC | MEMF_CLEAR);

    if (cd != NULL) {
        cd->cd_Rom.er_Product      = DEVICE_PROD;
        cd->cd_Rom.er_Flags        = ERFF_EXTENDED | ERFF_ZORRO_III;
//      cd->cd_Rom.er_Reserved03   = 0x00;
        cd->cd_Rom.er_Manufacturer = DEVICE_MFG;
//      cd->cd_Rom.er_SerialNumber = 0x00000000;
//      cd->cd_Rom.er_InitDiagVec  = 0x0000;
//      cd->cd_Rom.er_Reserved0c   = 0x00;
//      cd->cd_Rom.er_Reserved0d   = 0x00;
//      cd->cd_Rom.er_Reserved0e   = 0x00;
//      cd->cd_Rom.er_Reserved0f   = 0x00;

        cd->cd_Flags     = CDF_CONFIGME;
        if (firestorm_mode) {
            cd->cd_BoardAddr = (APTR) 0x40000000;
            cd->cd_BoardSize = 512 << 20; // 512 MB
            cd->cd_Rom.er_Type = ERT_ZORROIII | 0x05;  // 512 MB
        } else {
            /* AmigaPCI */
            struct ConfigDev *cd2;
            cd->cd_BoardAddr = (APTR) 0x80000000;
            cd->cd_BoardSize = 1024 << 20; // 1024 MB
            cd->cd_Rom.er_Type = ERT_ZORROIII | 0x06;  // 1 GB
            Disable();
            AddConfigDev(cd);
            Enable();
            cd2 = AllocMem(sizeof(struct ConfigDev), MEMF_PUBLIC | MEMF_CLEAR);
            CopyMem(cd, cd2, sizeof (*cd));
            cd = cd2;
            cd->cd_Rom.er_Manufacturer = 0xbee4;  // BEER
            cd->cd_Rom.er_Product = 'P';
            cd->cd_BoardAddr = (APTR) 0xc0000000;
            cd->cd_BoardSize = 992 << 20; // 992 MB (1024 MB - 16 MB)
        }
//      cd->cd_SlotAddr  = 0x0000;
//      cd->cd_SlotSize  = 0x0000;
//      cd->cd_Driver    = NULL;
//      cd->cd_NextCD    = NULL;
//      cd->cd_Unused[0] = 0x00000000;
//      cd->cd_Unused[1] = 0x00000000;
//      cd->cd_Unused[2] = 0x00000000;
//      cd->cd_Unused[3] = 0x00000000;

        /* Add the entry to the system list */
        Disable();
        AddConfigDev(cd);
        Enable();
        Printf("Fake AutoConfig entry added: Mfg $%04lx Prod $%02lx\n",
                DEVICE_MFG, DEVICE_PROD);
    }

done:
    CloseLibrary((struct Library *)ExpansionBase);
    return (0);
}
